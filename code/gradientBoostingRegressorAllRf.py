...
combine with RF takes it way down to 0.334390
...
# gini 0.312929 with 1000 estimators
...
# 0.400874  for 500 estimators

featureImportances = zip(train.columns, model2.feature_importances_)

featDf=pd.DataFrame(featureImportances)
featDf.to_clipboard()
...
rf = RF(n_jobs=-1)
rf.fit(train[feature_cols],train['Hazard'])
y_pred3 = rf.predict(train[feature_cols])

coef3 = giniscore.Gini(y_pred3,train['Hazard'])

print 'Gini coefficient is ', coef3

y_pred_all = (y_pred2 + y_pred3)/2.0
print 'Gini coefficient combined is ', giniscore.Gini(y_pred_all,train['Hazard'])
...
oos_pred3 = rf.predict(X_oos)

oos_all = (oos_pred + oos_pred3)/2.0

# create submission file
sub = pd.DataFrame({'Id':test.index, 'Hazard':oos_pred}).set_index('Id')
sub.to_csv('gradientBoostRegrHuber500estimators.csv')  

sub2 = pd.DataFrame({'Id':test.index, 'Hazard':oos_all}).set_index('Id')
sub2.to_csv('gradientBoostRegrHuber500estimatorsRf.csv')  
